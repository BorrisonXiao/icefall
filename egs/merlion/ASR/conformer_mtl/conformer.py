...
class ConformerMTL(Transformer):
    """
    Args:
        num_features (int): Number of input features
        num_classes (int): Number of output classes
    ...
    def __init__(
        ...
        if self.normalize_before:
            ...
            #       and throws an error without this change.
            self.after_norm = identity
        # Use lstm to produce one LID representation
        self.lstm = nn.LSTM(
            input_size=d_model,
            hidden_size=lid_dim,
        ...
        self.lid_head = nn.Linear(lid_dim, lid_num_classes)
    ...
    def run_encoder(
        ...
        # Using the cell output of the last layer of the lstm
        _, (_, lid_x) = self.lstm(x)
        lid_x = self.lid_head(lid_x)  # (1, B, LID_C)
    ...
    def forward(
        ...
        encoder_memory, lid_x, memory_key_padding_mask = self.run_encoder(
            x, supervision
        )
        x = self.ctc_output(encoder_memory)
        return x, lid_x, encoder_memory, memory_key_padding_mask
