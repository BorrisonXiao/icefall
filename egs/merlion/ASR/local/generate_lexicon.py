...
SPLITS = ["AISHELL", "NSC", "LibriSpeech", "SEAME", "dev"]
...
def generate_lexicon(manifests_dir: Path, lexicon: Path):
    ...
    assert len(manifests) == len(SPLITS), (
        len(manifests),
        list(manifests.keys())
    )
