...
def generate_lexicon(
    ...
    For a pure Chinese case, the lexicon is an identity mapping.
...
def main():
    ...
    # Step 1: Generate the symbol table based on the BPE model
    sp = spm.SentencePieceProcessor()
    sp.load(str(model_file))

    words = ["!SIL", "<SPOKEN_NOISE>", "<UNK>"] + [sp.id_to_piece(id).strip('▁') if len(
        sp.id_to_piece(id)) > 1 else sp.id_to_piece(id) for id in range(sp.get_piece_size())]
    excluded = ['<blk>', '<sos/eos>', '<unk>', '▁']
    for w in excluded:
        if w in words:
            words.remove(w)
