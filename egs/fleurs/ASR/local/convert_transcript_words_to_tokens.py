...
def get_args():
    ...
    parser.add_argument("--bpe-model", type=str,
                        help="The BPE model file. Note that this overwrite the lexicon and the oov.", default=None)
...
def process_line(
    ...
    Args:
      ...
      line:
        A line of transcript consisting of unseperated words.
      oov_token:
...
def main():
    ...
    # The BPE model will overwrite the lexicon and the oov
    if args.bpe_model is not None:
        ...
        with open(args.transcript) as f:
            for line in f:
                print(" ".join(sp.encode_as_pieces(line)).strip())
    else:
